...
class ChatContract(Protocol):
    ...
    The ``app`` parameter is an ``AlphaApp`` instance, ``agent`` is an ``Agent``
    ...
    async def setup(self) -> None:
        """Called by AlphaApp.setup() after all routers are mounted. Override to run async init (e.g. webhook registration)."""

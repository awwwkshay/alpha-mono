...
from clay_app.server.server import Server
from clay_app.workflow.workflow import Workflow
from clay_app.workspace.workspace import Workspace
from clay_core import AppConfig, AppContext, AppId, ServerConfig
...
class ClayApp:
    ...
    def _setup_tracing(self, config: AppConfig) -> None:
        ...
        if _owned_tracer_provider is not None:
            ...
            existing_service = _owned_tracer_provider.resource.attributes.get(
                "service.name", ""
            )
            ...
            if existing_service and existing_service != new_service:
                logger.warning(
                    ...
                    "Create apps with the same service_name or avoid multiple ClayApp "
                    "instances in the same process to prevent this."
                )
            else:
                logger.debug(
                    "Tracing already configured; reusing global tracer provider"
                )
            return
    ...
    async def __aenter__(self) -> ClayApp:
        await self.setup()
        return self

...
from clay_app.log import logger
from clay_app.constants.otel_constants import (
    AGENT_DESCRIPTION,
    AGENT_HAS_HISTORY,
    AGENT_HAS_SCORERS,
    AGENT_HAS_TOOLS,
...
from clay_app.evals.runner import run_scorers
from clay_core import AgentConfig, AgentTool, AppContext, ScorerResult
...
class Agent:
    ...
    def __init__(self, *, config: AgentConfig) -> None:
        from clay_app.workflow.workflow import Workflow as _Workflow
    ...
    async def _dispatch_tool(
        ...
        with _tracer.start_as_current_span(
            ...
            span.add_event(
                EVENT_TOOL_CALL,
                {"tool.name": name, "tool.input": json.dumps(arguments)},
            )
            try:
                if name in self._workflows:
                ...
                span.add_event(
                    EVENT_TOOL_RESULT, {"tool.name": name, "tool.output": result}
                )
                return result
    ...
    async def generate_with_evals_async(
        ...
        with _tracer.start_as_current_span(
            ...
            try:
                logger.info(
                    f"Agent '{self.config.name}' generating response with evals"
                )
                content, iterations, tool_calls = await self._generate_raw(
    ...
    async def generate_structured_async(
        ...
        with _tracer.start_as_current_span(
            ...
            try:
                logger.info(
                    f"Agent '{self.config.name}' generating structured response"
                )
                messages: list[AllMessageValues] = [
                    ChatCompletionSystemMessage(
                ...
                with _tracer.start_as_current_span(
                    ...
                    if finish_reason:
                        llm_span.set_attribute(
                            GEN_AI_RESPONSE_FINISH_REASON, finish_reason
                        )
            ...
            try:
                ...
                for iteration in range(_MAX_TOOL_ITERATIONS):
                    ...
                    if not collected_tool_calls:
                        ...
                        span.add_event(
                            EVENT_ASSISTANT_MESSAGE, {"content": final_content}
                        )
                        return

...
from clay_chat.schemas.slack import SlackAction, SlackCommand
...
class SlackAdapter:
    ...
    async def handle_event(self, event: dict[str, Any]) -> None:
        ...
        with _tracer.start_as_current_span(
            ...
            span.add_event(
                "chat.assistant.message", {"content": response, "channel": channel}
            )
            ...
            await self._slack_client.send_message(
                channel, response, thread_ts=thread_ts
            )
    ...
    async def handle_command(self, command: SlackCommand) -> None:
        ...
        with _tracer.start_as_current_span(
            ...
            span.set_attribute("user.prompt", text)
            span.add_event(
                "chat.user.message", {"content": text, "command": command.command}
            )
    ...
    async def handle_action(self, action: SlackAction) -> None:
        ...
        with _tracer.start_as_current_span(
            ...
            span.set_attribute("user.prompt", text)
            span.add_event(
                "chat.user.message", {"content": text, "action_type": action.type}
            )
